...
import { Lock, User, Mail, AlertCircle, Shield } from 'lucide-react';
import { Button } from '../components/ui/button';
import { useApp } from '../context/AppContext';
import { useLanguage } from '../context/LanguageContext';
import type { UserRole } from '../types';
import logoImage from 'figma:asset/c673552f7748d800c656a4166ccbeadb19bd4dd2.png';

export default function Login() {
  const navigate = useNavigate();
  const { login, environment } = useApp();
  const { t } = useLanguage();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (user && (password === 'demo123' || environment === 'Demo')) {
        login(user);
        navigate('/');
      } else {
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 border-4 border-white/20">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-[#01411C] mb-2">
                Email Address
              </label>
          ...
          <div className="mt-6 pt-6 border-t border-[#01411C]/10">
            <p className="text-xs text-center text-[#0B5D1E] mb-4">
              Demo Mode: Click any role to quick login
            </p>
            <div className="grid grid-cols-2 gap-2">
              {demoUsers.slice(0, 6).map((user) => (
                <Button
                  ...
                  className="text-[#01411C] border-[#01411C]/30 hover:bg-[#DFF5E1] text-xs py-2"
                >
          ...
          <div className="mt-6 text-center">
            <p className="text-xs text-[#0B5D1E]">
        ...
        <div className="mt-8 text-center text-white/70 text-sm">
          ...
          <p className="mt-2 text-xs">© 2026 Government of Pakistan</p>
        </div>
      </div>
    </div>
  );
