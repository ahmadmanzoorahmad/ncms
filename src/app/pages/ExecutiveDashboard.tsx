...
import { mockTickets, calculateKPIMetrics } from '../data/mock-data';
...
export default function ExecutiveDashboard() {
  const metrics = calculateKPIMetrics(mockTickets);
  ...
  const priorityData = [
    { name: 'Critical', value: mockTickets.filter(t => t.priority === 'Critical').length, color: '#DC3545' },
    { name: 'High', value: mockTickets.filter(t => t.priority === 'High').length, color: '#FFC107' },
    { name: 'Medium', value: mockTickets.filter(t => t.priority === 'Medium').length, color: '#0B5D1E' },
    { name: 'Low', value: mockTickets.filter(t => t.priority === 'Low').length, color: '#1F7A3A' },
  ];

  const categoryData = [
    { name: 'Technical', value: mockTickets.filter(t => t.category === 'Technical Issue').length },
    { name: 'Billing', value: mockTickets.filter(t => t.category === 'Billing').length },
    { name: 'Account', value: mockTickets.filter(t => t.category === 'Account Issue').length },
    { name: 'Bug', value: mockTickets.filter(t => t.category === 'Bug Report').length },
    { name: 'Feature', value: mockTickets.filter(t => t.category === 'Feature Request').length },
    { name: 'Other', value: mockTickets.filter(t => t.category === 'General Inquiry' || t.category === 'Other').length },
  ];
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="destructive">
                {mockTickets.filter(t => t.priority === 'Critical' && !['Closed', 'Auto-Resolved'].includes(t.status)).length}
              </Badge>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">At Risk SLA</span>
              <Badge className="bg-[#FFC107] text-black">
                {mockTickets.filter(t => t.slaStatus === 'At Risk').length}
              </Badge>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">Escalated</span>
              <Badge className="bg-[#FFC107] text-black">
                {mockTickets.filter(t => t.status === 'Escalated').length}
              </Badge>
            </div>
          </CardContent>
        </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center bg-white/10 backdrop-blur-sm rounded-lg p-4">
              <p className="text-4xl font-bold">
                {mockTickets.filter(t => t.isAIResolved).length}
              </p>
              <p className="text-sm text-white/90 mt-2">Total AI Resolutions</p>
            </div>
            <div className="text-center bg-white/10 backdrop-blur-sm rounded-lg p-4">
              <p className="text-4xl font-bold">
                ~{(mockTickets.filter(t => t.isAIResolved).length * 2).toFixed(0)}h
              </p>
